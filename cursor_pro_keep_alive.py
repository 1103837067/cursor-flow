...
from db_utils import DbAccountSaver  # 导入数据库保存工具
...
try:
    ...
    try:
        ...
        logging.info("数据库初始化成功，可以保存账号信息到远程数据库")
    except Exception as db_init_error:
        logging.error(f"数据库初始化失败，账号信息将不会被保存: {str(db_init_error)}")
        
except Exception as import_error:
    logging.error(f"导入数据库模块失败: {str(import_error)}")
    logging.warning("账号信息将不会被保存到远程数据库")
        ...
        try:
            ...
            for cookie in cookies:
                if cookie.get("name") == "WorkosCursorSessionToken":
                    ...
                    return token, user  # 返回token和user两个值
...
class EmailGenerator:
    ...
    def generate_email(self, length=4):
        ...
        self.email = f"{self.default_first_name}{timestamp}@{self.domain}"
        return self.email
    ...
    def save_account_info(self, user, token, total_usage):
        """保存账号信息到数据库"""
        try:
            # 使用db_utils中的工具类保存到数据库
            success = DbAccountSaver.save_account(
                self.email, self.default_password, user, token, total_usage
            )
            
            if success:
                logging.info(f"账号信息已成功保存到远程数据库")
            else:
                logging.error(f"保存账号信息到远程数据库失败")
            
            return success
        except Exception as e:
            logging.error(f"保存账号信息到数据库时发生错误: {str(e)}")
            return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("initializing_email_verification")
        ...
        if sign_up_account(browser, tab):
            ...
            token, user = get_cursor_session_token(tab)
            if token and user:
                # 更新本地认证信息
                ...
                # 保存账号信息到数据库
                logging.info("正在保存账号信息到远程数据库...")
                email_generator.save_account_info(user, token, "")
                sys.exit()
            else:
